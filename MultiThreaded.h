...
void findBestPerm(City* cities, unsigned int nCities, int** returnPermPtrPtr, unsigned int step=0, unsigned int offset=0) {
	...
	// set the return perm to an invalid address by default
	returnPermPtrPtr[offset] = nullptr;
	...
	do {
		...
		if (distance < bestDistance || bestDistance == -1) {
			...
			if (returnPermPtrPtr[offset]) {
				delete[] returnPermPtrPtr[offset];
			}

			// set the return perm to the current best perm
			returnPermPtrPtr[offset] = currentPerm;
		}
